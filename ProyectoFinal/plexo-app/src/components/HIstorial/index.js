  ...
  const handleViewOrderDetails = async (orderId) => {
    try {
      ...
      if (error.response && error.response.status === 404) {
        ...
        setError("Hubo un problema al cargar los detalles de la orden. Por favor, intenta de nuevo más tarde.");
      }
    }
  };
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        setError("Hubo un problema al cargar el historial de compras. Por favor, intenta de nuevo más tarde.");
      } finally {
        setLoading(false);
      }
    };
          ...
          <ul className="history-list">
            {history.map((purchase) => (
              <li key={purchase.order_id} className="history-item">
                ...
                <button className="details-button" onClick={() => toggleDetails(purchase.order_id)}>
